...
public class ImageUtils {
    ...
    public static void main(String[] args) {
        String imagePath = "D:\\桌面\\1.jpg";
        for (int i = 0; i < 1; i++) {
            try {
                Thumbnails.of(imagePath).scale(0.8).toFile(imagePath);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
...
//        generateDirectoryThumbnail();
    ...
    public static void generateDirectoryThumbnail(){
        try {
            Thumbnails.of(
                    new File("D:\\桌面\\独角兽\\独角兽2021app\\杜景涛").listFiles()).
                    scale(0.8).
                    toFiles(new File("D:\\桌面\\独角兽\\独角兽2021app\\陈冲\\new"), Rename.SUFFIX_HYPHEN_THUMBNAIL);
        } catch (IOException e) {
